...
module.exports = React.createClass({
	...
	render: function render() {
		...
		var separator = props.separator || React.createElement(
			'span',
			{ className: 'timepicker__separator', style: props.separatorStyle },
		...
		return React.createElement(
			'div',
			_extends({ className: 'timepicker' }, props),
			hour,
			hourSeparator,
			minute,
			minuteSeparator,
  ...
  */
	handleInputKeyDown: function handleInputKeyDown(props, name, event) {
		...
		if (name == 'meridian') {
			...
			if (isMeridianLetter || event.key == ' ' || event.key == 'Enter' || event.key == 'ArrowUp' || event.key == 'ArrowDown' || event.key == 'ArrowLeft' || event.key == 'ArrowRight') {
